	...
	function __dbt($text, $args = null) {
                ...
                if($text==null){
                    ...
                    if($languageUse!='en-us' && $languageUse!='eng')
                    {
                        ...
                        {
                            //return $text;
                        /****************Copy of locale __ function start*******************/
                        App::uses('I18n', 'I18n');
                        $translated = I18n::translate($text);
                        $arguments = func_get_args();
                        return I18n::insertArgs($translated, array_slice($arguments, 1));
                        /****************Copy of locale __ function end********************/
                           // return $text;//die('2');
                        }
                    }else{
                        App::uses('I18n', 'I18n');
...
//                    if($languageUse!='en-us' && $languageUse!='eng'){
//                        $modelName = "News";
//                        $modelNameLocale =  ucfirst($languageUse)."News";
//                        $model = ClassRegistry::init($modelName);
//                        $data = $model->find('first',array('fields'=>array($modelNameLocale.".*"),'conditions'=>array("News.description LIKE"=>"%$text%")));
//                        //prx($data);
//                        if(!empty($data[$modelNameLocale]) && $data[$modelNameLocale]['description']!=''){
//                            return strip_tags($data[$modelNameLocale]['description']);//die('1');
//                        }else{
//                            return strip_tags($text);//die('2');
//                        }
//                    }else{
	...
	function __dbtnews($textId) {
                if($textId==null){
                    ...
                    if($languageUse!='en-us' && $languageUse!='eng'){
                        ...
                        try 
                        {
                            $data = $model->find('first',array('fields'=>array("News.*",$modelNameLocale.".*"),'conditions'=>array("News.id"=>"$textId")));
                            //prx($data);
                        }
                        catch(Exception $e)
                        {
                            $modelName = "News";
                            $model = ClassRegistry::init($modelName);
                            $data = $model->find('first',array('fields'=>array("News.*"),'conditions'=>array("News.id"=>"$textId")));
                            //prx($data);
                            if(!empty($data["News"]) && $data["News"]['description']!=''){
                                return $data["News"]['description'];//die('1');
                            }
                        }
                        
                        if(isset($data[$modelNameLocale]) && !empty($data[$modelNameLocale]) && $data[$modelNameLocale]['description']!=''){
                            return $data[$modelNameLocale]['description'];//die('1');
                        }else{
                            return $data["News"]['description'];//die('2');
                        }
                    }else{
                        /********************************/
                        $modelName = "News";
