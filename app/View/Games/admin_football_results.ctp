...
<section class="content-header">
  <h1>
    ...
    <small><?php echo __dbt('Football Winners'); ?></small>
  </h1>
  ...
  <div class="row">
	...
	<div class="box">
		<div class="box-header">
			...
			<div class="box-body">	
			   <div class="row">
			     <?php  echo $this->Form->create('FootballPrediction', array('type' => 'get', 'url' => array('controller' => 'games', 'action' => 'footballResults', $this->params["prefix"] => true), "novalidate"=>"novalidate")); ?>
					    ...
					    <?php echo "<div class='col-xs-2'>". $this->Form->input("FootballPrediction.game_day", array("label"=>false, "div"=>false,"placeholder"=>"Search by Game",'empty'=>'Select Day', 'options' => @$game,'class'=>"form-control"))."</div>"; ?>
		...
		<div class="box-body">
			<table id="example" class="table table-striped">
				...
				<tbody>
				<?php  if(!empty($footballwinnerdata_array)) {
				    
                                    foreach ($footballwinnerdata_array as $footballs): 
									 foreach ($footballs as $football): ?>
					<tr>
						<!--td><!--?php // echo h($basketball['user']['name']); ?>&nbsp;</td-->
						<td><?php echo $this->Html->link($football['user']['name'],  'javascript:void(0)',array('class'=>'viewprofile','title'=>'View profile','userId' =>base64_encode($football['FootballPrediction']['user_id']))); ?></td>
						<td>
							<?php echo $this->Html->link($football['Tournament']['name'], array('controller' => 'tournaments', 'action' => 'view', base64_encode($football['GamesResult']['tournament_id']))); ?>
						</td>
						<td>
							<?php echo $this->Html->link($football['League']['name'], array('controller' => 'leagues', 'action' => 'view', base64_encode($football['GamesResult']['league_id']))); ?>
						</td>
						
						<td><?php echo h($football['GamesResult']['game_day']); ?>&nbsp;</td>
						<td class="actions">
							<?php echo $this->Html->link(__dbt(''),  'javascript:void(0)',array('class'=>'fa fa-eye viewresult','title'=>'View Game','userId' 
						=>base64_encode($football['FootballPrediction']['user_id']),'gameDay' => base64_encode($football['GamesResult']['game_day']))); ?>
