<?php  //echo '<pre>'; //print_r($soccerwinnerdata_array);?>
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1>
    <?php echo __dbt('Winners'); ?>
    <small><?php echo __dbt('Soccer Winners'); ?></small>
  </h1>
  ...
  <div class="row">
	...
	<div class="box">
		<div class="box-header">
			...
			<div class="box-body">	
			   <div class="row">
			     <?php  echo $this->Form->create('SoccerPrediction', array('type' => 'get', 'url' => array('controller' => 'games', 'action' => 'soccerResults', $this->params["prefix"] => true), "novalidate"=>"novalidate")); ?>
					    ...
					    <?php echo "<div class='col-xs-2'>". $this->Form->input("SoccerPrediction.league_id", array("label"=>false, "div"=>false,"placeholder"=>"Search by Game",'empty'=>'Select League', 'options' => @$league,'class'=>"form-control"))."</div>"; ?>
					      <?php echo "<div class='col-xs-2'>". $this->Form->input("SoccerPrediction.game_day", array("label"=>false, "div"=>false,"placeholder"=>"Search by Game",'empty'=>'Select Day', 'options' => @$game,'class'=>"form-control"))."</div>"; ?>
		...
		<div class="box-body">
			<table id="example" class="table table-striped">
				...
				<?php  if(!empty($soccerwinnerdata_array)) {
					...
					<tr>
						<td><?php echo $this->Html->link($soccer['user']['name'],  'javascript:void(0)',array('class'=>'viewprofile','title'=>'View profile','userId' =>base64_encode($soccer['soccerprediction']['user_id']))); ?></td>
						<td>
							<?php echo $this->Html->link($soccer['Tournament']['name'], array('controller' => 'tournaments', 'action' => 'view', base64_encode($soccer['GamesResult']['tournament_id']))); ?>
						</td>
						<td>
							<?php echo $this->Html->link($soccer['League']['name'], array('controller' => 'leagues', 'action' => 'view', base64_encode($soccer['GamesResult']['league_id']))); ?>
						</td>
						
						<td><?php echo h($soccer['GamesResult']['game_day']); ?>&nbsp;</td>
						<td class="actions">
							<?php echo $this->Html->link(__dbt(''),  'javascript:void(0)',array('class'=>'fa fa-eye viewresult','title'=>'View Game','userId' 
						=>base64_encode($soccer['soccerprediction']['user_id']),'gameDay' => base64_encode($soccer['GamesResult']['game_day']))); ?>
...
$(document).ready(function() {
   ...
   $('#example').DataTable( {
    ...
    $('.viewresult').click(function () {
        ...
        $('.modal-bodys').load(base_url+'/'+'admin/games/soccerWinnerResult/' + user_Id +'/' +  game_day, function (result) {
