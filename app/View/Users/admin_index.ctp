...
<section class="content-header">
  <h1>
    ...
    <small><?php echo __dbt('List Users'); ?></small>
  </h1>
  ...
  <div class="row">
	...
	<div class="box">
		...
		<div class="UserAdminIndexForm">
			<div class="box-body">	
			   <div class="row">
			     <?php echo $this->Form->create('User', array('type' => 'get', 'url' => array('controller' => 'users', 'action' => 'index', $this->params["prefix"] => true,'class'=>"form-control"), "novalidate"=>"novalidate")); ?>
				...
				<tr>
					...
					<td>
						...
						<?php echo $this->Html->link(__dbt(''), array('action' => 'edit', base64_encode($user['User']['id'])), array('class'=>'fa fa-edit','title'=>'Edit User')); ?>
                                                <?php if($user['User']['status_name'] == 'Active') { echo $this->Form->postLink(__dbt(''), array('action' => 'status', base64_encode($user['User']['id']),base64_encode(0)), array('class'=>'fa fa-user-times ','title'=>'Deactivate User'),array(__dbt('Are you sure you want to deactivate this user?')));
                                                } else {
                                                    echo $this->Form->postLink(__dbt(''), array('action' => 'status', base64_encode($user['User']['id']),base64_encode(1)), array('class'=>'fa fa-user-plus activate-icon','title'=>'Activate User'),array(__dbt('Are you sure you want to activate this user?')));
                                                }
?>
