				...
				<div class="box-body">	
				   <div class="row">
					...
					<?php echo $this->Form->create('Tournament', array('type' => 'get', 'url' => array('controller' => 'tournaments', 'action' => 'index', $this->params["prefix"] => true), "novalidate"=>"novalidate")); ?>
					    ...
					    <?php //echo "<div class='col-xs-3'>". $this->Form->input("League.sport_id", array("type"=>"select","label"=>false, "div"=>false,'class'=>"form-control","empty"=>"-- Select Sport --"))."</div>"; ?>  
			...
			<table class="table table-striped">
				...
				<?php if(!empty($tournaments)) { 
					...
					<tr>
						...
						<td class="actions">
							<?php echo $this->Html->link(__dbt(''), array('action' => 'view', base64_encode($tournament['Tournament']['id'])),array('class'=>'fa fa-eye','title'=>'View User')); ?>
							<?php echo $this->Html->link(__dbt(''), array('action' => 'edit', base64_encode($tournament['Tournament']['id'])),array('class'=>'fa fa-edit','title'=>'Edit User')); ?>
							<?php echo $this->Form->postLink(__dbt(''), array('action' => 'delete', base64_encode($tournament['Tournament']['id'])),array('class'=>'fa fa-remove','title'=>'Delete User'), array(__dbt('Are you sure you want to delete this tournament ?'))); ?>
						</td>
					</tr>
                                <?php endforeach; } else { ?>
                                    <tr>    
                                    <td class="text-center" colspan="3"><?php echo __dbt('No result found.'); ?>&nbsp;</td>
                                    </tr>     
                                <?php } ?>        
				</tbody>
				</table>
