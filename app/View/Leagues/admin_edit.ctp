				...
				<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								...
								<?php echo $this->Form->input('sport_id', array("class"=>"form-control",'label' => false, 'required'=>false, 'disabled' => true));  ?>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label" for="inputEmail3"><?php echo __dbt('Tournament'); ?></label>
							<div class="col-sm-4" id="tournament-box">
								<?php echo $this->Form->input('tournament_id', array("class"=>"form-control",'label' => false, "empty"=>"Please Select",'required'=>false, 'disabled' => true));  ?>
							</div>
						</div>
						<div class="form-group">
...
<div class="form-group">
							<label class="col-sm-2 control-label" for="inputEmail3"><?php echo __dbt('Order Number'); ?></label>
							<div class="col-sm-4">
								<?php
								
								for($i=1;$i<=10;$i++){$l[$i] = $i;}
								$result=array_diff($l,$orderArray);
								if($this->request->data['League']['order_number']!=11){$alrExist = $this->request->data['League']['order_number']; $result[$alrExist] = $alrExist;
								ksort($result);}
								
						echo $this->Form->input('order_number', array("type"=>"select", "options"=>$result,"class"=>"form-control",'label' => false,"empty"=>"Select order number"));  ?>
...
<script type="text/javascript">
		...
		jQuery('#LeagueTournamentId').val(jQuery('#LeagueTournamentId').children('option:nth-child(2)').val()).prop('disabled', true);
		});
	});
