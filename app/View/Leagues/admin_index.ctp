			...
			<div class="box-body">	
			   <div class="row">
			     <?php echo $this->Form->create('League', array('type' => 'get', 'url' => array('controller' => 'leagues', 'action' => 'index', $this->params["prefix"] => true), "novalidate"=>"novalidate")); ?>
					    ...
					    <?php echo "<div class='col-xs-2'>". $this->Form->input("League.host", array("label"=>false, "placeholder"=>"Search by Host", "div"=>false,'class'=>"form-control"))."</div>"; ?>
			...
			<table class="table table-striped">
				...
				<?php if(!empty($leagues)) {
					...
					<tr>
						...
						<td>
							<?php echo $this->Html->link($league['Sport']['name'], array('controller' => 'sports', 'action' => 'view', base64_encode($league['Sport']['id']))); ?>
						</td>
						...
						<td>
						 ...
						 <td><?php if($league['League']['status']==1){echo "Active";}else{ echo "Inactive";} ?>&nbsp;</td>
						<td class="actions">
							<?php echo $this->Html->link(__dbt(''), array('action' => 'view', base64_encode($league['League']['id'])),array('class'=>'fa fa-eye','title'=>'View League')); ?>
							<?php echo $this->Html->link(__dbt(''), array('action' => 'edit', base64_encode($league['League']['id'])),array('class'=>'fa fa-edit','title'=>'Edit League')); ?>
							<?php echo $this->Form->postLink(__dbt(''), array('action' => 'delete', base64_encode($league['League']['id'])),array('class'=>'fa fa-remove','title'=>'Delete League'), array(__dbt('Are you sure you want to delete this league?'))); ?>
						</td>
					</tr>
                                <?php endforeach; } else { ?>
                                    <tr>    
                                    <td class="text-center" colspan="9"><?php echo __dbt('No result found.'); ?>&nbsp;</td>
                                    </tr>
                                <?php } ?>        
				</tbody>
				</table>
